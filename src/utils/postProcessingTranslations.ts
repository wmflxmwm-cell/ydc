...
// 후공정 설명 번역 매핑
export const postProcessingDescriptionTranslations: Record<string, { ko: string; vi: string }> = {
...
export const translatePostProcessingDescription = (description: string | undefined, language: 'ko' | 'vi'): string | undefined => {
  ...
  const translation = postProcessingDescriptionTranslations[description];
  if (translation) {
    return translation[language];
  }
